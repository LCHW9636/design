$(document).ready(function(){
   ...
   이전에 오버했던 li를 계산하는 것보다 
   모든 li의 active를 지워버림. 그리고 오버한 li에만 다시 active클래스를 줌*/
   ...
   $('.interview .list ul li').on('mouseenter', function(){
    ...
    /* pc버전에서 메뉴에 마우스를 오버하면 hader에 menu_over 클래스 추가
    ...
    /* 모바일 메뉴
    .header .gnb .gnb_open을 클릭하며 ㄴ메뉴가 열림
            -- header에 menu_open클래스 추가
    .header .gnb .gnb_close 를 클릭하면 메뉴가 닫힘
                --header에 menu_open 클래스 삭제
    1차 메뉴를 클릭하면 하위메뉴 열리는데 해당 li에 sub_+open 클래스를 추가1차 메뉴 클릭
