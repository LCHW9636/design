$(document).ready(function(){
    ...
    let offset = 200 // 콘텐츠가 하단에서부터 offset값만큼 올라오면 애니메이션 값 줌
    ...
    function scroll_chk(){
        ...
        sec04_obj_top = $('.sec05').offset().top
        ...
        if((sec02_obj_top - scrolling - window_h) < -offset){
            ...
            $('.sec02 .photo').width(500 - (sec02_obj_top - scrolling - window_h + offset)*1.5)
            if((500 - (sec02_obj_top - scrolling - window_h + offset)) <= $(window).width()){
                $('.sec02 .photo').width(500 - (sec02_obj_top - scrolling - window_h + offset)*1.5)
        ...
        if((sec05_obj_top - scrolling - window_h) < -offset){
            ...
            $('.sec05 .tit').attr('style', 'transform: translate(0, '+ sec05_offset +'-500''px);')
        }else{
