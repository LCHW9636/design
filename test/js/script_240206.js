//alert('Helloworld!')
// 확인창 alert - 잘 사용안함
console.log('개발자모드에서만 확인 가능한 메시지 출력')

var a = '한글' //a라는 변수에 '한글'이라는 값을 저장
a = '대한' //기존에 저장된 값을 지우고 다시 '대한'만 저장함
var b = '민국'
a = a + b
console.log(a)
...
// let abc = '민국' 이미 사용중인 변수를 또 선언하면 에러가 나옴
abc = '중복'
console.log(abc)
...
let box_w = $('.box').width()
console.log(box_w)
/* js는 대부분 head태그안에서 불러들임 근데 js에서 html요소를 불러서 효과를 줌
근데 아직 hhtml요소가 로딩되기 전임
<head>
    -- js파일
</head>
<body>
...
이런 구조이기 때문에 js가 불렸을 때에는 html은 로딩되기도 전임...

jquery는 호출될때 단 한 번 작동됨.
근데 jquery는 조건이 될때 다시 한 번 호출이 되도록 하는 기능이 있음
*/
...
$(document).ready(function(){ //html 문서가 모두 로딩된 이후에 1번 실행 
    ...
    $('.box').on('mouseenter', function(){ //마우스를 오버했을때
        //box 자기 자신한테 active 클래스를 줌
        // $('box').addclass('active') - 모든 박스에 class가 추가됨
        $(this).addClass('active') //마우스를 오버한 자기 자신
    })
    $('.box').on('mouseleave', function(){
        $(this).removeClass('active') 
            /* box에 마우스를 오버하면 active 클래스를 추가
    box에서 마우수를 아웃하면 active 클래스를 삭제*/
    })

    //box1이 1초동안 opacity가 0으로 점점 변하는 애니메이션 
    $('.box1').animate({
        opacity : 0
    }, 1000)
